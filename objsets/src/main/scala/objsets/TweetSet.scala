...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val filteredLeftSubtree = left.filterAcc(p, acc)
    if (p(elem)) right.filterAcc(p, filteredLeftSubtree.incl(elem))
    else right.filterAcc(p, filteredLeftSubtree)
  }

  def union(that: TweetSet): TweetSet = that.unionAcc(this.unionAcc(new Empty))

  def unionAcc(acc: TweetSet): TweetSet = {
    val elemUnion = acc.incl(elem)
    val leftUnion = left.unionAcc(elemUnion)
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter((tweet: Tweet) => google.exists(tweet.text.contains))
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter((tweet: Tweet) => apple.exists(tweet.text.contains))
