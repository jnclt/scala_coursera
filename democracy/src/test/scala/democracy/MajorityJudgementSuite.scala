...
class MajorityJudgementSuite extends FunSuite:
  ...
  test("The median of an empty collection is undefined") {
    ...
    catch case NonFatal(_) => () // OK, all fine
  }

  test(
    "The median of a collection with a single element should be that element"
  ) {
  ...
  test(
    "The median of a collection with two elements can be any of those two elements"
  ) {
    val grade1, grade2 = randomGrade()
    val median         = Grade.median(List(grade1, grade2))
    val (min, max) =
      if grade1.ordinal < grade2.ordinal then (grade1, grade2)
      else (grade2, grade1)
    assert(
  ...
  test(
    ...
    do
      ...
      firstHalf.find(_.ordinal > medianGrade.ordinal).foreach {
        counterExample =>
          fail(
      ...
      secondHalf.find(_.ordinal < medianGrade.ordinal).foreach {
        counterExample =>
          fail(
  ...
  test(
    "A median value minimizes the arithmetic mean of the absolute deviations of a collection"
  ) {
    for
      _ <- 1 to 100
      length = Random.nextInt(50) + 1
      ...
      medianErrors = grades
        .map(value => math.abs(median.ordinal - value.ordinal))
        .sum
  ...
  test(
    ...
    do
      ...
      assert(
        loosers.forall(candidate =>
          candidateMedians(candidate).ordinal <= highestMedian.ordinal
        )
      )
  }
  ...
  test("full scenario with no tie-break") {
    ...
    val election =
      Election("Best dessert", Set(tiramisu, cremeBrulee, cheesecake))
  ...
  test(
    "full scenario with a tie-break and the same grades for cooking and hiking"
  ) {
    val karting = Candidate("Karting")
    val sailing = Candidate("Sailing")
    val hiking  = Candidate("Hiking")
